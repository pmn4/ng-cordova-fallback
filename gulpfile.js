...
gulp.task('build', function () {
  ...
  // create modules file programatically
  // create injector file programatically



  return gulp.src(buildConfig.pluginFiles)
    .pipe(concat('ng-cordova-fallback.js'))
    .pipe(header(buildConfig.closureStart))
    .pipe(footer(buildConfig.closureEnd))
    .pipe(header(buildConfig.banner))
    .pipe(gulp.dest(buildConfig.dist))
    // .pipe(gulp.dest(buildConfig.demo.ngCordovaFallback))
    .pipe(uglify())
    .pipe(header(buildConfig.banner))
...
function makeChangelog(version) {
  ...
  changelog({
    repository: 'https://github.com/pmn4/ng-cordova-fallback',
    version: version,
    file: file
  }, function (err, log) {
